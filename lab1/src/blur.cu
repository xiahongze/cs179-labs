                                  ...
                                  const unsigned int blur_v_size)
{
    ...
    const uint j_max = min(thread_index + 1, blur_v_size);
...
float cuda_call_blur_kernel(const unsigned int blocks,
                            ...
                            const unsigned int blur_v_size)
{
    const unsigned int array_size = sizeof(float) * n_frames;
    ...
    float *gpu_raw_data;
    cudaMalloc((void **)&gpu_raw_data, array_size);
    cudaMemcpy(gpu_raw_data, raw_data, array_size, cudaMemcpyHostToDevice);
    ...
    float *gpu_out_data;
    cudaMalloc(&gpu_out_data, array_size);
    cudaMemset(gpu_out_data, 0, array_size);
    ...
    cudaMemcpy(out_data, gpu_out_data, array_size, cudaMemcpyDeviceToHost);
